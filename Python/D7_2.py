...
def main(DATA_INPUT):
    ...
    # following the order, and using add, multiply and combining current_value with the next operand
    def calculate(current_value, operands):
        if len(operands) == 0:
            ...
            return calculate(current_value + operands[0], operands[1:]) or \
                calculate(current_value * operands[0], operands[1:]) or \
                calculate(int(str(current_value) + str(operands[0])), operands[1:])
